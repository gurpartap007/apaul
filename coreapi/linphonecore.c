...
//static pid_t child_pid_I;
extern SalCallbacks linphone_sal_callbacks;
int gpio_set_value(struct gpio_descriptor_t* gpio_desc, unsigned int val)
{
	int setval_fd;
	sprintf(gpio_desc->buf, "/sys/class/gpio/gpio%d/value",
			gpio_desc->gpio_num);
	//printf("value of gpio %s \t --> %d\n", gpio_desc->buf,val);
	setval_fd = open(gpio_desc->buf, O_WRONLY);
	if (val)
		write(setval_fd, "1", 2);
	else
		write(setval_fd, "0", 2);
...
LinphoneCall * linphone_core_invite_address_with_params(LinphoneCore *lc, const LinphoneAddress *addr, const LinphoneCallParams *params)
{
	...
	void linphone_core_notify_incoming_call(LinphoneCore *lc, LinphoneCall *call){
		...
		int i=0;
		char *tmp;
		char *total_command,*kill_command;
		char *host,*ip;
		char *program_name="/home/lcd/caller";
		int ret;
		...
		memset(host,'\0',10);
		memset(ip,'\0',20);
		for(count=0,i=0;count<(strlen(tmp));count++)
		{
			if(tmp[count]=='@')
			{
out:
		...
		total_command = malloc((strlen(program_name))+(strlen(host))+(strlen(ip))+5);
		kill_command = malloc((strlen(program_name))+20);
		sprintf(kill_command,"kill -2 `pidof %s`",program_name);
		ms_message("kill command = %s\n",kill_command);
		sprintf(total_command,"%s %s %s &",program_name,host,ip);
		ms_message("total_command = %s\n",total_command);
		//	printf("total_command  = %s\n",total_command);
		ret=system(total_command);
		//	ret=system(kill_command);
		//sleep(5);
		//ret=system("/home/lcd/caller`");
	...
	const char *linphone_reason_to_string(LinphoneReason err){
		switch(err){
			...
			case LinphoneReasonNoResponse:
				//system("/home/lcd/busy &");
				//system("kill -2 `pidof /home/lcd/busy`");
				//system("kill -2 `pidof /home/lcd/busy`");
				return "No response";
			case LinphoneReasonForbidden:
				return "Bad credentials";
			case LinphoneReasonDeclined:
				system("/home/lcd/declined &");
				system("kill -2 `pidof /home/lcd/declined`");
				system("kill -2 `pidof /home/lcd/declined`");
