...
#include "lpconfig.h"
//#include "/root/new_linphone/linphone/console/linphonec.c"
// stat
#ifndef _WIN32
#include <sys/types.h>
#include <sys/stat.h>
			...
			if (call->state!=LinphoneCallUpdating){
				if (call->state==LinphoneCallResuming){
				...
				}else{
					{
						...
						//system("kill -2 `pidof /home/lcd/connected`");
						//system("kill -2 `pidof /home/lcd/connected`");
						linphone_core_notify_display_status(lc,msg);
						ms_free(tmp);
						ms_free(msg);
...
//	linphonec_out("call terminated on lcd");
	if (call->refer_pending){
		linphone_core_start_refered_call(lc,call,NULL);
	}
	...
	//system("kill -20 `pidof /home/lcd/caller`");
