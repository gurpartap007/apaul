...
lpc_cmd_call(LinphoneCore *lc, char *args)
{
	...
	char *total_command;
	char *kill_command;
	char *program_name="/home/lcd/calling";
	int ret,count,i;
	if ( ! args || ! *args )
	{
		return 0;
	}
	{
		...
		else
		{
			...
			///////////////////  calling name extraction  /////////////////////

			host = malloc(10);
			ip   = malloc(20);
			memset(host,'\0',10);
			memset(ip,'\0',20);
			for(count=0,i=0;count<(strlen(callee_name));count++)
			{
				if(callee_name[count]=='@')
				{
out:
					ip[i] = callee_name[count+1];
					i++;count++;
					if(callee_name[count]=='\0')
						break;
					goto out;
				}
				host[count]=callee_name[count];
...
lpc_cmd_terminate(LinphoneCore *lc, char *args)
{
	...
	//system("kill -2 `pidof /home/lcd/terminated`");
	//system("kill -2 `pidof /home/lcd/terminated`");
...
{
	int ret;	
	ret=system("/home/lcd/quit &");
	ret=system("kill -2 `pidof /home/lcd/quit`");
	ret=system("kill -2 `pidof /home/lcd/quit`");
